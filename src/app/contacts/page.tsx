...
const mockContacts = [
  { id: 1, name: 'Alice Builder', email: 'alice@example.com', phone: '+44 7123 456789', status: 'New', source: 'Web', company: 'BuildCo' },
  { id: 2, name: 'Bob Loft', email: 'bob@example.com', phone: '+44 7987 654321', status: 'Qualified', source: 'Referral', company: 'LoftLtd' },
  { id: 3, name: 'Carol Plans', email: 'carol@example.com', phone: '+44 7456 123789', status: 'Proposal', source: 'Email', company: 'PlansPro' },
  { id: 4, name: 'David Ext', email: 'david@example.com', phone: '+44 7321 987654', status: 'Won', source: 'Phone', company: 'ExtCorp' },
];
    ...
    <main className="min-h-screen flex bg-slate-100">
      ...
      <section className="flex-1 flex flex-col ml-60">
        <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between">
          ...
          <button onClick={() => setShowForm(!showForm)} className="bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-emerald-800">
            + Add Contact
          </button>
        </header>
          ...
          {showForm && (
            <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={addContact} className="bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold">Save Contact</button>
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b last:border-0 hover:bg-slate-50">
                    ...
                    <td className="py-3">
                      <span className={"text-xs px-2 py-0.5 rounded-full font-medium " + (statusColors[c.status] || 'bg-slate-100 text-slate-700')}>
                        {c.status}
                      </span>
                    </td>
                    <td className="py-3 text-slate-500">{c.source}</td>
                    <td className="py-3">
                      <button onClick={() => deleteContact(c.id)} className="text-xs text-red-500 hover:text-red-700 font-medium">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
